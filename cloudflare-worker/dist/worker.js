...
var PUBLIC_API_BASE = "https://contact-page-editor.deem0u.workers.dev";
...
__name(getAdminBaseUrl, "getAdminBaseUrl");
...
__name(parsePagePath, "parsePagePath");
...
__name(getContactPageCountForUser, "getContactPageCountForUser");
var corsHeaders = {
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (request.method === "GET" && path.startsWith("/api/profile/")) {
        ...
        let username = raw || "";
        try {
          if (raw) username = decodeURIComponent(raw);
        } catch (_) {
        }
...
__name(generateOneTimePassword, "generateOneTimePassword");
...
var EMAIL_RESTRICTION_RECIPIENT = "deem0u.github.io@gmail.com";
...
__name(shouldDivertEmail, "shouldDivertEmail");
...
async function handleRecover(request, env) {
  ...
  if (code) {
    ...
    const { recoveryCode: recoveryCode2, expiresAt: expiresAt2 } = JSON.parse(storedData);
    if (Date.now() > expiresAt2) {
      await env.EDIT_KEYS_KV.delete("admin:recovery_code");
      return jsonResponse({ error: "Code expired. Please request a new one." }, 400);
    }
    if (code !== recoveryCode2) {
    ...
    const resetExpiresAt = Date.now() + 60 * 60 * 1e3;
    ...
    const subject2 = "Admin Dashboard - Password Reset Link";
    const text2 = `Click the link below to set a new password. The link expires in 1 hour.

${resetLink}

If you did not request this, please ignore this email.`;
    const emailResult2 = await sendEmail(env, { to: storedEmail, subject: subject2, text: text2 });
    if (!emailResult2.ok) {
...
function generateRecoveryCode() {
  ...
  const code = (array[0] << 16 | array[1] << 8 | array[2]) % 1e6;
...
async function handleAuth(request, env) {
  ...
  if (email && password) {
    ...
    const storedKey2 = await env.EDIT_KEYS_KV.get("admin:key");
    return jsonResponse({ success: true, adminKey: storedKey2 });
  }
...
async function handleAuthUser(request, env) {
  ...
  if (storedOtp && storedOtp === password) {
    const exp2 = Math.floor(Date.now() / 1e3) + JWT_EXPIRY_DAYS * 86400;
    const token2 = await signJwt({ username, exp: exp2, otpLogin: true }, secret);
    return jsonResponse({ success: true, username, token: token2, mustSetPassword: true });
  }
...
__name(handleCheckAccountEmail, "handleCheckAccountEmail");
async function handleSignup(request, env) {
  ...
  const secret = env.JWT_SECRET || env.SESSION_SECRET;
  const token = secret ? await signJwt({ username, exp: Math.floor(Date.now() / 1e3) + JWT_EXPIRY_DAYS * 86400 }, secret) : null;
  return jsonResponse({
    success: true,
    username,
...
__name(handleOtpSend, "handleOtpSend");
...
__name(handleSignupSuccessEmail, "handleSignupSuccessEmail");
var CONTACT_FORM_RECIPIENT = "deem0u.github.io@gmail.com";
...
__name(normalizeDob, "normalizeDob");
function generateContactPageHTML(givenName, familyName, contactEmail, mobile, mobileLink, homeCountry, destName, destAddress, destPhone, destEmail, additionalInfo) {
  const esc = /* @__PURE__ */ __name((s) => (s ?? "").toString().replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;"), "esc");
  ...
  const formatMultiline = /* @__PURE__ */ __name((s) => {
  ...
  const titles = "Last updated \xB7 Derni\xE8re mise \xE0 jour \xB7 \u6700\u540E\u66F4\u65B0";
  const sectionTitle = "Contact Information \xB7 Coordonn\xE9es \xB7 \u8054\u7CFB\u4FE1\u606F";
  const lblGiven = "Given Names \xB7 Pr\xE9noms \xB7 \u540D\u5B57";
  const lblFamily = "Family Name \xB7 Nom de famille \xB7 \u59D3\u6C0F";
  const lblEmail = "Email \xB7 Courriel \xB7 \u7535\u5B50\u90AE\u4EF6";
  const lblMobile = "Contact Number \xB7 T\xE9l\xE9phone \xB7 \u7535\u8BDD";
  const lblCountry = "Home Country \xB7 Pays de r\xE9sidence \xB7 \u5C45\u4F4F\u56FD\u5BB6";
  const lblDest = "Destination Details \xB7 D\xE9tails de la destination \xB7 \u76EE\u7684\u5730\u8BE6\u60C5";
  const lblAdditional = "Additional Information \xB7 Informations suppl\xE9mentaires \xB7 \u9644\u52A0\u4FE1\u606F";
...
__name(generateContactPageHTML, "generateContactPageHTML");
...
__name(requireEmailVerifiedForEdit, "requireEmailVerifiedForEdit");
...
__name(handleUserCreatePage, "handleUserCreatePage");
...
__name(handleDeletePage, "handleDeletePage");
...
__name(handleAdminRenameUser, "handleAdminRenameUser");
...
__name(handleGetSiteStatus, "handleGetSiteStatus");
...
__name(handleGetSiteSettings, "handleGetSiteSettings");
...
__name(handlePutSiteSettings, "handlePutSiteSettings");
async function getValidUsernamesFromGitHub(env) {
  ...
  const contents = await response.json();
  const usernames = (Array.isArray(contents) ? contents : []).filter((item) => item.type === "dir" && item.name && !item.name.startsWith(".")).map((item) => item.name);
  const valid = [];
...
__name(getValidUsernamesFromGitHub, "getValidUsernamesFromGitHub");
...
__name(listAllKvKeys, "listAllKvKeys");
...
__name(collectKvOrphans, "collectKvOrphans");
...
__name(handleGetKvOrphans, "handleGetKvOrphans");
...
__name(collectKvUserKeys, "collectKvUserKeys");
...
__name(handleGetKvUserKeys, "handleGetKvUserKeys");
async function handleKvUserPurge(request, env) {
  if (!await isAdmin(request, env)) return jsonResponse({ error: "Admin access required" }, 401);
  if (!env.EDIT_KEYS_KV) return jsonResponse({ error: "KV not configured" }, 500);
  let body;
  try {
...
__name(handleKvUserPurge, "handleKvUserPurge");
...
__name(handleGetAdminEmail, "handleGetAdminEmail");
...
__name(handleInternalSetAdminCredentials, "handleInternalSetAdminCredentials");
...
__name(handlePutAdminEmail, "handlePutAdminEmail");
...
__name(handlePutAdminKey, "handlePutAdminKey");
...
__name(handleVerifyOtpSetPassword, "handleVerifyOtpSetPassword");
...
__name(handlePutAdminPassword, "handlePutAdminPassword");
...
__name(handleAdminGenerateOtp, "handleAdminGenerateOtp");
...
__name(handleAdminDeleteOtp, "handleAdminDeleteOtp");
...
__name(handleResetPasswordWithToken, "handleResetPasswordWithToken");
...
__name(handleGetPage, "handleGetPage");
...
__name(handleUpdatePage, "handleUpdatePage");
...
__name(handleRenamePage, "handleRenamePage");
...
__name(handleListPages, "handleListPages");
async function handlePageSummaries(request, env) {
  ...
  const fetchSummary = /* @__PURE__ */ __name(async (username) => {
    try {
...
__name(handlePageSummaries, "handlePageSummaries");
async function handleListContactPages(username, request, env) {
  ...
  const contents = await response.json();
  const files = Array.isArray(contents) ? contents : [];
  const slugs = files.filter((item) => item.type === "file" && item.name && item.name.endsWith(".html")).map((item) => item.name.replace(/\.html$/i, ""));
  const contactPages = [];
  for (const slug of slugs) {
    const name = env.EDIT_KEYS_KV ? await env.EDIT_KEYS_KV.get(`contact_page_name:${uLower}:${slug}`) || (slug === "index" ? "Main (index)" : slug) : slug === "index" ? "Main (index)" : slug;
...
__name(handleListContactPages, "handleListContactPages");
...
__name(handleGetAccountEmails, "handleGetAccountEmails");
async function handleGetAccountProfiles(request, env) {
  ...
  const addSuffix = /* @__PURE__ */ __name((keyName, prefix) => {
...
__name(handleGetAccountProfiles, "handleGetAccountProfiles");
async function handleDebugUser(username, request, env) {
  ...
  if (recoveryRaw) {
    try {
      ...
      recoverySqCount = Array.isArray(r.secretQuestions) ? r.secretQuestions.length : 0;
    } catch (_) {
    }
  }
  ...
  const validUsernames = await getValidUsernamesFromGitHub(env);
  const validSet = new Set(validUsernames.map((x) => x.toLowerCase()));
  const inGitHub = validSet.has(uLower);
  return jsonResponse({
    debug: {
      username: u,
      ...
      dobSet,
      recoverySqCount,
      contactPageKeysCount: contactNameKeys.length,
      toFolderEntries,
      matchingAccountEmailKeys
    }
  });
}
__name(handleDebugUser, "handleDebugUser");
...
__name(handleSecretsStatus, "handleSecretsStatus");
...
__name(handleGetKeys, "handleGetKeys");
...
__name(handleAccountDetailsSent, "handleAccountDetailsSent");
...
__name(handleRecoveryCheck, "handleRecoveryCheck");
...
__name(handleRecoveryCheckByEmail, "handleRecoveryCheckByEmail");
async function handleRecoveryVerify(request, env) {
  ...
  if (questionIds && Array.isArray(questionIds) && answers && Array.isArray(answers)) {
    ...
    const dobNorm2 = normalizeDob(dobRaw);
    if (!dobNorm2) return jsonResponse({ error: "Recovery verification failed" }, 401);
    const storedDob2 = await env.EDIT_KEYS_KV.get("user_dob:" + username);
    if (!storedDob2 || storedDob2.trim() !== dobNorm2) return jsonResponse({ error: "Recovery verification failed" }, 401);
    const recoveryRaw2 = await env.EDIT_KEYS_KV.get("user_recovery:" + username);
    let secretQuestions2 = [];
    if (recoveryRaw2) {
      try {
        const r = JSON.parse(recoveryRaw2);
        secretQuestions2 = Array.isArray(r.secretQuestions) ? r.secretQuestions : [];
      } catch (_) {
      }
    }
    for (let i = 0; i < 2; i++) {
      ...
      const q2 = secretQuestions2.find((x) => x && String(x.questionId) === String(qid));
      if (!q2 || (q2.answer || "").trim().toLowerCase() !== ans.toLowerCase()) return jsonResponse({ error: "Recovery verification failed" }, 401);
    }
    await env.EDIT_KEYS_KV.delete("account_details_sent:" + username);
    const otp2 = generateOneTimePassword();
    await env.EDIT_KEYS_KV.put("user_otp:" + username, otp2);
    const toEmail2 = (await env.EDIT_KEYS_KV.get("account_email:" + username) || "").trim();
    if (toEmail2 && toEmail2.includes("@")) {
      const subject = "Your one-time password - Digital Contact Page";
      const text = `Your one-time sign-in password is: ${otp2}

Use this to sign in at My Account (with your Account Email or User Name). You will then be asked to set a new permanent password.

If you did not request this, please contact support.`;
      const html = `<p>Your one-time sign-in password is: <strong>${otp2}</strong></p><p>Use this to sign in at My Account (with your Account Email or User Name). You will then be asked to set a new permanent password.</p><p>If you did not request this, please contact support.</p>`;
      await sendEmail(env, { to: toEmail2, subject, text, html, username });
    }
...
__name(handleRecoveryVerify, "handleRecoveryVerify");
...
__name(handleResetAccess, "handleResetAccess");
...
__name(handlePutProfile, "handlePutProfile");
async function handleVerifyEmailChange(request, env) {
  ...
  let newEmail;
  try {
    newEmail = JSON.parse(pendingRaw).newEmail;
  } catch (_) {
    return jsonResponse({ error: "Invalid pending data" }, 400);
  }
...
__name(handleVerifyEmailChange, "handleVerifyEmailChange");
...
__name(handleCancelEmailChange, "handleCancelEmailChange");
...
__name(handleSetPasswordAfterOtp, "handleSetPasswordAfterOtp");
...
__name(handleChangePasswordAndSq, "handleChangePasswordAndSq");
...
__name(handleProfileRename, "handleProfileRename");
...
__name(handleGetSecrets, "handleGetSecrets");
...
__name(handlePutSecrets, "handlePutSecrets");
...
function extractSummaryFromHtml(html) {
  const esc = /* @__PURE__ */ __name((s) => String(s).replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "esc");
  const g = /* @__PURE__ */ __name((label) => {
...
__name(extractSummaryFromHtml, "extractSummaryFromHtml");
export {
